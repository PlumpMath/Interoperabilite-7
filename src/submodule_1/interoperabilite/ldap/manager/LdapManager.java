...
public class LdapManager {
    ...
    private LdapManager() {
        ...
        env.put(Context.INITIAL_CONTEXT_FACTORY, LdapConfiguration.getInstance().getLdapContextFactory());
        env.put(Context.PROVIDER_URL, LdapConfiguration.getInstance().getLdapServerUrl() + "/"
                + LdapConfiguration.getInstance().getLdapBaseDN());
        env.put(Context.SECURITY_AUTHENTICATION, LdapConfiguration.getInstance().getLdapAuthentificationMode());
        env.put(Context.SECURITY_PRINCIPAL, LdapConfiguration.getInstance().getLdapUser());
        env.put(Context.SECURITY_CREDENTIALS, LdapConfiguration.getInstance().getLdapPassword());
    ...
    public ArrayList<LdapItem> request(String ouSearch, Boolean isDebug) {
        try {
            ...
            // Filtre du LDAP
            NamingEnumeration<SearchResult> userAnswer = ctx.search(ouSearch, null);
    ...
    public void requestInspector(LdapItem item, String ouTree, Boolean isDebug) {
        try {
            if (item.getOu() != null && !item.getOu().equals("")) {
                ...
                ctx = new InitialLdapContext(env, null);
                NamingEnumeration<SearchResult> userAnswer = ctx.search(ouTree, null);
                ctx.close();
    ...
    private ArrayList<LdapItem> itemExport(NamingEnumeration<SearchResult> userAnswer, Boolean isDebug)
            ...
            if (attrs != null) {
                ...
                try {
                    ...
                    for (NamingEnumeration ae = attrs.getAll(); ae.hasMore();) {
                        ...
                        } else if (attr.getID().equals("isMemberOfPartialAttributeSet")) {
                            if (isDebug) {
                                System.out.println("isMemberOfPartialAttributeSet " + attr.get(0).toString());
                            }
                            item.setIsMemberOfPartialAttributeSet(attr.get(0).toString());
                        } else if (attr.getID().equals("systemFlags")) {
                            if (isDebug) {
                                System.out.println("systemFlags " + attr.get(0).toString());
                            }
                            item.setSystemFlags(attr.get(0).toString());
                        } else if (attr.getID().equals("adminCount")) {
                            if (isDebug) {
                                System.out.println("adminCount " + attr.get(0).toString());
                            }
                            item.setAdminCount(attr.get(0).toString());
                        } else if (attr.getID().equals("fSMORoleOwner")) {
                            if (isDebug) {
                                System.out.println("fSMORoleOwner " + attr.get(0).toString());
                            }
                            item.setfSMORoleOwner(attr.get(0).toString());
                        } else {
                            item.setOther(attr.get(0).toString());
                            if (isDebug) {
                                System.out.println("other " + attr.get(0).toString());
                            }
                        }
                    }
    ...
    public SearchResult findAccountByAccountName(String ldapSearchBase, String accountName) throws NamingException {
    ...
    public String findGroupBySID(String ldapSearchBase, String sid) throws NamingException {
    ...
    public ArrayList<Student> getStudentsByPromotion(Promotion promotion) {
        ...
        for (LdapItem user_ldapitem : items) {
            promotion.getUsers().add(new Student(0, user_ldapitem.getName(), user_ldapitem.getGivenName(), "", "",
                    user_ldapitem.getObjectGUID(), null));
        }
    ...
    public ArrayList<Promotion> getAllPromotions(String school) {
        String requestBase = "OU=Eleves,OU=Utilisateurs,OU=Formation,OU=" + school + ",OU=Sites";
    ...
    public ArrayList<Promotion> getPromotions(String school, String years, String promotion) {
        String requestBase = "OU=Eleves,OU=Utilisateurs,OU=Formation,OU=" + school + ",OU=Sites";
        ArrayList<LdapItem> items = this.request(requestBase, false);
    ...
    public ArrayList<Teacher> getAllTeachers(String school){
        //OU=Formateurs_Externes,OU=Utilisateurs,OU=Formation,OU=RENNES,OU=Sites,DC=imie,DC=lan
        String requestBase = "OU=Formateurs_externes,OU=Utilisateurs,OU=Formation,OU=" + school + ",OU=Sites";
        ArrayList<LdapItem> items = this.request(requestBase, false);
        ArrayList<Teacher> teachers = new ArrayList<Teacher>();
        
        for (LdapItem ldapItem : items) {
            Teacher teacher = new Teacher(0, ldapItem.getName(),ldapItem.getGivenName(), "", "", ldapItem.getObjectGUID(), null );
